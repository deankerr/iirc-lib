...
function msg(
  ...
  const m: IrcMessage = { tags, command, params }
  if (source !== undefined) m.source = source
  return m
}

const from: ParsedSource = { name: 'nick', user: 'u', host: 'h', isSelf: false }
...
describe('buildEvent', () => {
  describe('param() — required middle params', () => {
    test('PRIVMSG extracts target and text', () => {
      ...
      if (event.command !== 'PRIVMSG') return
  ...
  describe('trailing() — trailing params', () => {
    test('NOTICE puts text in trailing field', () => {
      ...
      if (event.command !== 'NOTICE') return
  ...
  describe('numeric resolution', () => {
    test('001 resolves to RPL_WELCOME and extracts fields', () => {
      ...
      if (event.command !== 'RPL_WELCOME') return
...
describe('runtime parse_error', () => {
  test('emits parse_error when enricher throws on missing required param', () => {
    ...
    runtime.on('parse_error', (message, error) => parseErrors.push({ message, error }))
